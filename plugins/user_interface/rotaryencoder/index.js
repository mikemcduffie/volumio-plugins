...
const detentActionType = Object.freeze({
  "NO_ACTION": 0,
  "VOLUME": 1,
  "PREVNEXT": 2,
  "SEEK": 3,
...
const buttonActionType = Object.freeze({
  "NO_ACTION": 0,
  "PLAY": 1,
  "PAUSE": 2,
  "PLAYPAUSE": 3,
...
rotaryencoder.prototype.onStart = function() {
  ...
  if (self.config.get('third_encoder_CLK') !== undefined && self.config.get('third_encoder_SW') !== 0)
    self.constructThirdEncoder(true);
...
rotaryencoder.prototype.getUIConfig = function() {
  ...
  var encodingOpts = fs.readJsonSync((__dirname + '/options/encodingOptions.json'), 'utf8', {
    throws: false
  });
  var detentOpts = fs.readJsonSync((__dirname + '/options/detentOptions.json'), 'utf8', {
    throws: false
  });
  var buttonOpts = fs.readJsonSync((__dirname + '/options/buttonOptions.json'), 'utf8', {
    throws: false
  });
  ...
  self.commandRouter.i18nJson(__dirname + '/i18n/strings_' + lang_code + '.json',
    ...
    .then(function(uiconf) {
      self.logger.info('[Rotary encoder] Loading settings page...');

      // Global settings
      uiconf.sections[0].content[0].value = self.config.get('longPressThresholdInMilliseconds');
      self.logger.info("[Rotary encoder] 1/5 settings loaded");
      ...
      self.logger.info("[Rotary encoder] 2/5 settings loaded");
      ...
      self.logger.info("[Rotary encoder] 3/5 settings loaded");

      // Third encoder
      uiconf.sections[3].content[0].value = self.config.get('third_encoder_CLK');
      uiconf.sections[3].content[1].value = self.config.get('third_encoder_DT');
      uiconf.sections[3].content[2].value = self.config.get('third_encoder_SW');
      ...
      for (var n = 0; n < encodingOpts.encodings.length; n++) {
        ...
        if (encodingOpts.encodings[n].enc == parseInt(self.config.get('third_encoder_encoding'))) {
          uiconf.sections[3].content[3].value.value = encodingOpts.encodings[n].enc;
          uiconf.sections[3].content[3].value.label = encodingOpts.encodings[n].label;
        }
      }
      ...
      for (var n = 0; n < detentOpts.detentActionTypes.length; n++) {
        ...
        if (detentOpts.detentActionTypes[n].type == parseInt(self.config.get('third_encoder_detentActionType'))) {
          uiconf.sections[3].content[4].value.value = detentOpts.detentActionTypes[n].type;
          uiconf.sections[3].content[4].value.label = detentOpts.detentActionTypes[n].label;
        }
      }

      for (var n = 0; n < buttonOpts.buttonActionTypes.length; n++) {
        self.configManager.pushUIConfigParam(uiconf, 'sections[3].content[5].options', {
          value: buttonOpts.buttonActionTypes[n].type,
          label: buttonOpts.buttonActionTypes[n].label
        });
        self.configManager.pushUIConfigParam(uiconf, 'sections[3].content[6].options', {
          value: buttonOpts.buttonActionTypes[n].type,
          label: buttonOpts.buttonActionTypes[n].label
        });

        if (buttonOpts.buttonActionTypes[n].type == parseInt(self.config.get('third_encoder_buttonActionType'))) {
          uiconf.sections[3].content[5].value.value = buttonOpts.buttonActionTypes[n].type;
          uiconf.sections[3].content[5].value.label = buttonOpts.buttonActionTypes[n].label;
        }
        if (buttonOpts.buttonActionTypes[n].type == parseInt(self.config.get('third_encoder_longPressActionType'))) {
          uiconf.sections[3].content[6].value.value = buttonOpts.buttonActionTypes[n].type;
          uiconf.sections[3].content[6].value.label = buttonOpts.buttonActionTypes[n].label;
        }
      }
      self.logger.info("[Rotary encoder] 4/5 settings loaded");

      uiconf.sections[4].content[0].value = self.config.get('enable_debug_logging');
      self.logger.info("[Rotary encoder] 5/5 settings loaded");
...
rotaryencoder.prototype.constructThirdEncoder = function() {
  var self = this;
  try {
    this.thirdEncoder = rotaryEncoder(self.config.get('third_encoder_CLK'), self.config.get('third_encoder_DT'), self.config.get('third_encoder_SW'), self.config.get('third_encoder_encoding'));
  } catch (ex) {
    self.logger.info('[Rotary encoder] Could not initiate rotary encoder #3 with error: ' + ex);
  }

  this.thirdEncoder.on('rotation', direction => {
    if (direction > 0) {
      if (self.config.get('enable_debug_logging') == true)
        self.logger.info('[Rotary encoder] Encoder #3 rotated right');

      if (self.config.get('third_encoder_detentActionType') == detentActionType.NO_ACTION && !self.config.get('enable_debug_logging'))
        self.logger.info('[Rotary encoder] Encoder #3 rotated right');

      if (self.config.get('third_encoder_detentActionType') == detentActionType.VOLUME)
        self.executeCommand('volume plus');
      else if (self.config.get('third_encoder_detentActionType') == detentActionType.SEEK)
        self.executeCommand('seek plus');
      else if (self.config.get('third_encoder_detentActionType') == detentActionType.PREVNEXT)
        self.executeCommand('next');
      else if (self.config.get('third_encoder_detentActionType') == detentActionType.SCROLL)
        self.emitToSocket('scroll', 'down');
    } else {
      if (self.config.get('enable_debug_logging'))
        self.logger.info('[Rotary encoder] Encoder #3 rotated left');
  ...
  if (self.config.get('third_encoder_SW') !== 0) {
    self.thirdEncoder.on('click', pressState => {
      if (self.config.get('enable_debug_logging'))
        self.logger.info('[Rotary encoder] Encoder #3 button pressed; press state = ' + (pressState == 0 ? 'pressed' : 'released'));

      var released;
      if (pressState == 1) {
        released = new Date();
        if ((released - self.pressed) >= self.config.get('longPressThresholdInMilliseconds')) {
          if (self.config.get('third_encoder_longPressActionType') != buttonActionType.NO_ACTION) {
            if (self.config.get('third_encoder_longPressActionType') != buttonActionType.SHUTDOWN && self.config.get('third_encoder_longPressActionType') != buttonActionType.REBOOT)
              self.executeCommand(self.determineAPICommand(self.config.get('third_encoder_longPressActionType')));
            else {
              if (self.config.get('third_encoder_longPressActionType') == buttonActionType.SHUTDOWN)
                self.commandRouter.shutdown();
              else
                self.commandRouter.reboot();
            }
          ...
          if (self.config.get('third_encoder_buttonActionType') != buttonActionType.NO_ACTION) {
            if (self.config.get('third_encoder_buttonActionType') != buttonActionType.SHUTDOWN && self.config.get('third_encoder_buttonActionType') != buttonActionType.REBOOT)
              self.executeCommand(self.determineAPICommand(self.config.get('third_encoder_buttonActionType')));
            else {
              if (self.config.get('third_encoder_buttonActionType') == buttonActionType.SHUTDOWN)
                self.commandRouter.shutdown();
              else
                self.commandRouter.reboot();
            }
...
rotaryencoder.prototype.destroyThirdEncoder = function() {
  var self = this;
  var defer = libQ.defer();
  try {
    if (this.thirdEncoder != undefined)
      this.thirdEncoder.destroy();
    defer.resolve();
  } catch (ex) {
    self.logger.info('[Rotary encoder] Could not deinit rotary encoder #3 with error: ' + ex);
    defer.reject(new Error());
  }
...
rotaryencoder.prototype.updateThirdEncoder = function(data) {
  var self = this;
  var defer = libQ.defer();

  self.destroyThirdEncoder()
    .then(function(updateConf) {
      self.config.set('third_encoder_CLK', parseInt(data['third_encoder_CLK']));
      self.config.set('third_encoder_DT', parseInt(data['third_encoder_DT']));
      self.config.set('third_encoder_SW', parseInt(data['third_encoder_SW']));
      self.config.set('third_encoder_encoding', parseInt(data['third_encoder_encoding'].value));
      self.config.set('third_encoder_detentActionType', parseInt(data['third_encoder_detentActionType'].value));
      self.config.set('third_encoder_buttonActionType', parseInt(data['third_encoder_buttonActionType'].value));
      self.config.set('third_encoder_longPressActionType', parseInt(data['third_encoder_longPressActionType'].value));
    })
    .then(function(rebuild) {
      self.constructThirdEncoder();
      defer.resolve(rebuild);
    })
    .fail(function() {
      defer.reject(new Error());
  ...
  if (self.config.get('enable_debug_logging'))
    self.logger.info('[Rotary encoder] Saved configuration for encoder #3, data: ' + JSON.stringify(data));
  self.commandRouter.pushToastMessage('success', "Saved settings", "Successfully saved third encoder settings.");
